...
struct Context {
    // Keep a parser in the context because it has to recompile a bunch
    // of regexes each time it is instantiated
    lease_parser: lease::Parser,
    settings: config::Settings,
}

impl Context {
    fn new(settings: config::Settings) -> Self {
        let lease_parser = lease::Parser::new();
        Self {
            lease_parser,
            settings,
        }
...
fn leases_handler(ctx: &Context) -> Response {
    let leases = ctx.lease_parser.parse_file(&ctx.settings.leases_file);
